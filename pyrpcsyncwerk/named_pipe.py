...
class NamedPipeTransport(RpcsyncwerkTransport):
    """
    ...
    in lib/rpcsyncwerk-named-pipe-transport.[ch] files.
...
class NamedPipeClient(RpcsyncwerkClient):
...
class NamedPipeServer(object):
    """
    Rpcsyncwerk server based on named pipe transport. Note this server is
...
class PipeHandlerThread(Thread):
    ...
    def run(self):
        while True:
            ...
            data = json.loads(req)
            resp = rpcsyncwerk_server.call_function(data['service'], data['request'])
            # logger.info('resp is %s', resp)
